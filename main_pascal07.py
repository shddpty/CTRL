...
def calculate_loss(loss_fn, pred, label, inc_L_ind, data, rec_v, inc_V_ind, beta, mu, logvar, zs):
    views = len(data)
    mse_loss = loss_fn.weighted_mse_loss(inc_V_ind, data, rec_v)
    beta_loss = loss_fn.beta_loss_function(pred, label, inc_L_ind)
    ...
    for i in range(views):
        for j in range(views):
            if i == j:
                continue
            contra_loss.append(loss_fn.weighted_contrastive_loss(zs[:, i, :], zs[:, j, :], inc_V_ind[:, i], inc_V_ind[:, j]))
    contra_loss_val = beta * torch.mean(torch.tensor(contra_loss)) if len(contra_loss) > 0 else torch.tensor(0.0).to(mse_loss.device)

    KLD = torch.mean(-0.5 * torch.sum((1 + logvar - mu.pow(2) - logvar.exp()) * inc_V_ind.unsqueeze(-1), dim=2)) * 1e-3

    total_loss = beta_loss + mse_loss + KLD + contra_loss_val
    return total_loss
...
def _test_first(loader,  model, epoch, logger, mode='train'):
    ...
    with torch.no_grad():
        for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
            ...
            pred, _, _, _, _ = model(data, inc_V_ind)
            # pred = pred.cpu()
            pred = pred['probability'].cpu()
...
def train_first(loader, model, loss_fn, optimizer, scheduler, epoch, logger, beta):
    losses = AverageMeter()
    model.train()
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        pred_s, rec_s, mu_s, logvar_s, zs = model(data, inc_V_ind)

        if epoch > 0:
            index = ((pred_s['belief'] > b_thread) & (pred_s['uncertainty'] < u_thread)).to(device).detach()
            label = label * inc_L_ind + pred_s['belief'].detach() * index * inc_L_ind.logical_not()
            inc_L_ind = inc_L_ind.logical_or(index)

        s_loss_u = calculate_loss(loss_fn, pred_s, label, inc_L_ind, data, rec_s, inc_V_ind, beta, mu_s, logvar_s, zs)
        optimizer.zero_grad()
        s_loss_u.backward()
        optimizer.step()
        loss = s_loss_u
...
def main(args, file_path):
    ...
    if args.logs:
        logfile = osp.join(args.logs_dir, str(b_thread) + "_" + str(u_thread) + args.name + "main_" + args.dataset + '_V_' + str(
    ...
    for fold_idx in range(folds_num):
        ...
        initialize(model)
        loss_fn = Loss(args.alpha, args.gamma, args.ev_reg).to(device)
        ...
        for epoch in range(args.epochs):
            train_losses_first,  model = train_first(train_dataloder,  model, loss_fn, optimizer, scheduler, epoch, logger, args.beta)
            with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets', type=list, default=['ODIR'])
    ...
    parser.add_argument('--use_soft_pseudo_labels', action='store_true', help='使用软伪标签替代硬伪标签')
    parser.add_argument('--ev_reg', type=float, default=0.1, help='证据正则化强度，控制惩罚不正确样本上高置信度的程度')
    ...
    args.datasets = ['pascal07']  # corel5k pascal07 espgame mirflickr iaprtc12
            ...
            for beta in beta_list:
                ...
                for gamma in gamma_list:
                    ...
                    for d_emb in d_emb_list:
                        args.d_emb = d_emb
                        for u in u_thread_list:
                            u_thread = u
                            for b in b_thread_list:
                                b_thread = b
                                        ...
                                        for training_ratio in training_ratio_list:
                                            ...
                                            for dataset in args.datasets:
                                                ...
                                                file_path = osp.join(args.records_dir,
                                                                     str(b_thread) + "_" + str(u_thread) + args.name + args.dataset + '_ViewMask_' + str(
